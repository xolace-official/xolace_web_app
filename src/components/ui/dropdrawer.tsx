...
function DropDrawerContent({
  ...
  const transition = {
    duration: 0.3,
    ease: [0.25, 0.1, 0.25, 1.0] as Easing, // cubic-bezier easing
  };
  ...
  if (isMobile) {
    ...
    const content = (
      ...
      <div
        ...
        className={cn(
          "flex cursor-pointer items-center justify-between px-4 py-4 text-secondary-foreground",
          // Only apply margin, background and rounded corners if not in a group
          !isInsideGroup && "bg-accent dark:bg-primary mx-2 my-1.5 rounded-md",
